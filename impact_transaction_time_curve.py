...
def analyse_linear(trades: pd.DataFrame, function: str, model: str, exponent: float = 0.5):
    """Fit I_i/Q^exponent ~ a·TradedVolume + b and save the ratio plot."""
...
def analyse_flat(trades: pd.DataFrame, function: str, model: str, exponent: float = 0.5):
    """Fit I_i/(Σq_i)^exponent ~ Y (constant) plotted against TradedVolume."""
    print('\n── Flat fit ──')
...
def analyse_by_nb_child(trades: pd.DataFrame, function: str, model: str,
    ...
    """Plot I_i/Q^exponent vs TradedVolume for each NbChild value."""
    print('\n── NbChild Analysis ──')
...
if __name__ == '__main__':
    ...
    a_lin, b_lin, a_lin_err, b_lin_err = analyse_linear(trades, function, model, exponent=exponent)

    analyse_by_nb_child(trades, function, model, exponent=exponent)
    a_lin5, b_lin5, a_lin5_err, b_lin5_err = analyse_linear_min_child(trades, function, model, min_child=5, exponent=exponent)
    a_lin10, b_lin10, a_lin10_err, b_lin10_err = analyse_linear_min_child(trades, function, model, min_child=10, exponent=exponent)
